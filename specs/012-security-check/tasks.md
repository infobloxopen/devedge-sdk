...
- [X] T001 [S] Write `seccheck/seccheck_test.go` — unit tests for static assertions (must be
  red until T002):
...
- [X] T002 [S] Create stub `seccheck/seccheck.go` (package declaration + exported types only,
...
- [X] T003 [S] Implement `seccheck/seccheck.go` fully (FR-001/005/006):
...
- [X] T004 [S] Implement `seccheck/dynamic.go` (FR-002/003/004):
...
- [X] T005 [S] Implement `cmd/security-check/main.go` (FR-007):
...
- [X] T006 [S] Write `testdata/toy/security_test.go` (SC-005):
...
- [X] T007 [S] Add `security-check` target to `Makefile`:
  ```makefile
...
- [X] T008 [S] `go build ./... && go vet ./... && make test` from the repo root — all clean (SC-006).

- [X] T009 [S] `cd testdata/toy && go test -run Security -v -count=1 -timeout 30s` — SC-005 passes.

- [X] T010 [S] Commit all: spec + plan + tasks + implementation.
