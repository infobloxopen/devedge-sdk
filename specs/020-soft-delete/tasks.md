...
- [ ] [S] T001: In `render.go`, add `SoftDelete bool` and `HasExpireTime bool`
  fields to the `messageInfo` struct (with doc comments). No behavior change yet.
  (FR-003)

- [ ] [C] T002: In `main.go` `generateFile`, detect the soft-delete and TTL
...
- [ ] [C] T003: In `render.go` `renderMessage`, gate the trailing model fields:
...
- [ ] [C] T004: In `render.go` `renderStorageFile`, add conditional imports —
...
- [ ] [C] T005: In `render.go` `fromModel_<Msg>`, populate
...
- [ ] [S] T006: In `render.go`, add column-map entries: `"delete_time":
...
- [ ] [C] T007: In `render.go`, rewrite generated `Delete`: capture
...
- [ ] [C] T008: In `render.go` generated `List`, when `msg.SoftDelete` emit
...
- [ ] [C] T009: In `render.go`, emit a generated `Undelete(ctx, key string)`
...
- [ ] [C] T010: In `render.go`, emit a generated `PurgeExpired(ctx, before
...
- [ ] [C] T011: In `render_test.go`, flip `TestRenderStorageFile_basic`:
...
- [ ] [C] T012: In `persistence/repository.go`, add
...
- [ ] [C] T013: In `persistence/memory.go`, implement soft-delete: add
...
- [ ] [S] T014: In `persistence/memory_test.go`, add `TestMemoryRepository_SoftDelete`
...
- [ ] [C] T015: In `persistence/entrepo/repository.go`, add
...
- [ ] [C] T016: In `persistence/entrepo/mixin.go`, add `SoftDeleteMixin`
...
- [ ] [S] T017: In `persistence/entrepo/mixin_test.go`, add `SoftDeleteMixin`
...
- [ ] [C] T018: In `cmd/protoc-gen-ent`, when a message declares a `delete_time`
...
- [ ] [C] T019: In `seccheck/dynamic.go`, add optional
...
- [ ] [S] T020: Update `testdata/toy/widgets.proto`: import
...
- [ ] [S] T021: Update `testdata/apikey/apikey.proto`: import
...
- [ ] [C] T022: Rebuild the plugins (`make generate` builds
...
- [ ] [C] T023: In `testdata/apikey/apikeyv1/ent_wiring.go`, add the `Undelete_`
...
- [ ] [C] T024: Add a GORM integration test (new
...
- [ ] [C] T025: Extend `testdata/apikey/apikeyv1/security_isolation_test.go`:
...
- [ ] [C] T026: Run `make build && make test` in the root module,
