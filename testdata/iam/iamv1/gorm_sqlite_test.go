...
// gorm_sqlite_test.go — a self-contained GORM SQLite dialector for the IAM GORM
// events test (events_gorm_test.go). It mirrors the inline dialector the other
// GORM fixtures use (testdata/apikey .../security_isolation_test.go): we cannot
// import gorm.io/driver/sqlite (it blank-imports the CGo mattn driver), so this
// uses only gorm.io/gorm sub-packages plus the modernc.org/sqlite driver,
// registered under "sqlite3". (The existing sqlite_test.go registers the same
// "sqlite3" alias for ent; the registration here is guarded and idempotent.)
...
func registerSQLite3() {
	registerSQLite3Once.Do(func() {
		if slices.Contains(sql.Drivers(), testSQLiteDriverName) {
			return
		}
		db, err := sql.Open("sqlite", ":memory:")
		if err != nil {
			panic("gormtx test setup: open sqlite driver: " + err.Error())
		}
		drv := db.Driver()
		_ = db.Close()
...
func (d *testSQLiteDialector) BindVarTo(writer clause.Writer, _ *gorm.Statement, _ any) {
	writer.WriteByte('?')
}
...
func (d *testSQLiteDialector) Explain(sql string, vars ...any) string {
